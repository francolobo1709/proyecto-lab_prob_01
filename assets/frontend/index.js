...
// ==================== INTEGRACIÓN SISTEMA DE IMÁGENES ====================

// Función para refrescar imágenes cuando cambia la página
function refreshPageImages(pageId) {
    if (window.imageManager) {
        // Dar tiempo a que el DOM se actualice
        setTimeout(() => {
            window.imageManager.replacePlaceholders();
        }, 100);
    }
}

// Función para actualizar galería de producto
function updateProductGallery(productId) {
    ...
    galleryThumbs.forEach((thumb, index) => {
        thumb.addEventListener('click', () => {
            ...
            // Actualizar imagen principal con la imagen de la galería
            const galleryUrls = window.imageManager.getImageUrl('products', productId, 'gallery');
            if (galleryUrls && galleryUrls[index]) {
                if (mainImage) {
                    window.imageManager.setRealImage(mainImage, galleryUrls[index], `${productId} vista ${index + 1}`);
                }
            }
        });
    });
...
// Función para obtener estadísticas del sistema de imágenes
...
// Función para precargar imágenes de una página específica
function preloadPageImages(pageId) {
    if (!window.imageManager) return;
    
    const pageElement = document.getElementById(pageId);
    if (!pageElement) return;
    
    const imageElements = pageElement.querySelectorAll('[data-product-id]');
    imageElements.forEach(element => {
        const productId = element.dataset.productId;
        const category = element.dataset.category || 'products';
        
        if (productId) {
            window.imageManager.loadImageWithFallback(element, category, productId);
        }
    });
}

// Ejecutar cuando el sistema de imágenes esté listo
document.addEventListener('DOMContentLoaded', () => {
    // Configurar listeners para galería cuando se carga una página de producto
    const originalShowProductDetail = window.showProductDetail;
    window.showProductDetail = function(productId) {
        originalShowProductDetail(productId);
        
        // Configurar galería después de mostrar el producto
        setTimeout(() => {
            updateProductGallery(productId);
            refreshPageImages(productId);
        }, 200);
    };
    
    // Configurar refresh de imágenes cuando cambia de página
    const originalShowPage = window.showPage;
    window.showPage = function(pageId) {
        originalShowPage(pageId);
        
        // Refrescar imágenes después de cambiar página
        setTimeout(() => {
            refreshPageImages(pageId);
        }, 100);
    };
...
window.addEventListener('load', function() {
    ...
    setTimeout(() => {
        ...
        // Inicializar sistema de imágenes después de que todo esté cargado
        if (window.imageManager) {
            refreshPageImages('home');
        }
    }, 100);
});
